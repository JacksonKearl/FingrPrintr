...
export const FingerprintJS = (function (exports) {
	'use strict'
	 ...
	 */
	async function forEachWithBreaks(
		items,
		callback,
	...
	function getAudioFingerprint() {
		const w = window
		const AudioContext =
			w.OfflineAudioContext || w.webkitOfflineAudioContext
		if (!AudioContext) {
		...
		return withIframe((_, { document }) => {
			...
			for (let index = 0; index < baseFonts.length; index++) {
				defaultWidth[baseFonts[index]] =
					baseFontsSpans[index].offsetWidth // width for the default font
				defaultHeight[baseFonts[index]] =
					baseFontsSpans[index].offsetHeight // height for the default font
			}
	...
	const sources = {
		...
		// begin sources from Fingerprintjs2 1.0.0-rc3  https://github.com/Valve/fingerprintjs2
	...
	function getUserAgentFingerprint() {
		// exclude low bits of version info for better stability
		return navigator.userAgent.replace(/\/([.\d])[.\d]+/g, '')
	}

	// end sources from Fingerprintjs2
